...
function Camera(scene){
  ...
  this.checkFocusBounds = function(){
    var centerX = this.target.x + (this.target.width/2);
    var centerY = this.target.y + (this.target.height/2);
...
function Tile( mapX, mapY, x, y, type ){
  ...
  this.checkCollision = function( sprite, w, h ){
    var shw = sprite.width/2;
    var shh = sprite.height/2;
    var scx = sprite.x + shw;
    var scy = sprite.y + shh;
    var thw = w/2;
    var thh = h/2;
    var tcx = this.x + thw;
    var tcy = this.y + thh;
...
function TileMap(scene){
  ...
  this.loadTileSheet = function(tileWidth, tileHeight, sheetWidth, sheetHeight, tileSheet, tileSymbols){
    ...
    var i, numRows = Math.floor(this.SheetWidth/this.tileWidth);
    var j, numCols = Math.floor(this.SheetHeight/this.tileHeight);
    for(i = 0; i < numRows; i++){
      for(j = 0; j < numCols; j++){
  ...
  this.drawMap = function(){//this could be WAY faster
    this.camera.update();
    var i, j, ctx = this.camera.context;
      ...
      for(j = 0; j < this.mapData[i].length; j++){ //for each column of each row
        ...
        if( 0 < drawX < this.camera.cWidth && 0 < drawY < this.camera.cHeight ){
          ...
          if( this.tiles[i][j].animationPlaying ){ 
            this.drawTileAnimation(this.tiles[i][j], ctx); 
          }
          else{
            ctx.drawImage(this.tileSheet, sheetX, sheetY, 
  ...
  this.loadCollisionMap = function( collisionMap ){
    // tile Symbol and collision Callback - - NOTE: This function will overwrite specific Collision Callbacks
    //convert collisionMap symbols to their associated integers
    var l, c, notConverted; 
  ...
  this.checkCollisions = function(sprite){ //check for collisions between sprite and tile
    var i, tileCoordX = Math.floor( sprite.x/this.tileWidth );
    var j, tileCoordY = Math.floor( sprite.y/this.tileHeight );
    var checkRowsBegin = tileCoordX - 1;
    var checkRowsEnd = tileCoordX + 2;
    var checkColsBegin = tileCoordY - 1;
    var checkColsEnd = tileCoordY + 2;
